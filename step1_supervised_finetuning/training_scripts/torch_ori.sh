...
run_benchmark() {
    ...
    do
        ...
        do
            ...
            # 提示当前运行的任务信息
            echo "Running PyTorch Benchmark: Model=${MODEL}, BS=${BS}, L=${L}, R=${R}, O=${O}"

            # 清除环境变量
            unset PYTORCH_CUDA_ALLOC_CONF

            # 调用 finetune.sh 脚本，不启用 GMLake
            bash training_scripts/finetune.sh ${GPU_NUM} ${BS} ${MODEL} False ${L} ${R} ${O}
        done
    done
}
